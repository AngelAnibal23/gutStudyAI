  ...
  useEffect(() => {
    fetch(`${API}/tareas`)
      ...
      .then(tareas => {
        ...
        // Pre-marcar checks para tareas ya completadas en Supabase
        if (programacion?.bloques) {
          const completadas = new Set(
            tareas.filter(t => t.estado === 'completada').map(t => t.id)
          );
          const checksDeCompletadas = {};
          programacion.bloques.forEach((bloque, idx) => {
            if (bloque.tarea_id && completadas.has(bloque.tarea_id)) {
              checksDeCompletadas[bloque.programacion_tarea_id ?? idx] = true;
            }
          });
          if (Object.keys(checksDeCompletadas).length > 0) {
            setChecks(prev => ({ ...prev, ...checksDeCompletadas }));
          }
        }
      })
      .catch(() => { });
